...
# for frozen diffusion
from sd import StableDiffusion, StableDiffusionModel, add_tokens_to_model_from_path

def main():

    # I love lovely_tensors
    import lovely_tensors # tensor를 더 보기 좋게 출력해주는 라이브러리
    lovely_tensors.monkey_patch()

    # Arguments, command line으로 입력받은 인자들
    opt = Options().parse_args()
    seed_everything(opt.seed)
    print(opt)
    
    # 인자로 받은 workspace 경로에 디렉토리가 없을 경우 만들기
    Path(opt.workspace).mkdir(exist_ok=(opt.ckpt != 'scratch'), parents=True)
    
    # workspace 경로에 command line 인자들, 실행 옵션들 저장
    ...
    # Create model, NeRF로 Instant-NGP 사용
