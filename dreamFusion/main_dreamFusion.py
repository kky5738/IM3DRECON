...
def main(image, category):
    ...
    # preprocessing image
    # preprocess(image) # testing, 나중에 주석 해제
    print('image has preprocessed') # testing
    
    # workspace = { # 나중에 주석 해제
    #     'chair': 'dmtet_chair',
    #     '2': 'dmtet_2',
    # }
    workspace = dict([(str(key+1), f'dmtet_{key+1}') for key in range(0, 5)]) # testing
    image = image.split('.')[0] + '_rgba.png'
    main_args = ['-O', '--image', image, '--workspace', workspace[category], '--dmtet', '--test', '--save_mesh']
    opt = parser.parse_args(args=main_args)

    if opt.O:
        print('opt.O is True') # testing
        opt.fp16 = True
        opt.cuda_ray = True

    elif opt.O2:
        print('opt.O2 is True')  # testing
        opt.fp16 = True
        opt.backbone = 'vanilla'

    # parameters for image-conditioned generation
    if opt.image is not None:
        print(f'"opt.imgae is not None" is True; opt.image={opt.image}') # testing
        
        if opt.text is None:
            # use zero123 guidance model when only providing image
            print('"opt.text is None" is True') # testing
            ...
            print('"opt.text is None" is False') # testing
    ...
    if opt.dmtet:
        print('"opt.dmtet" is True') # testing
        ...
        if opt.guidance != 'zero123':
            ...
            print('"opt.guidance != \'zero123\'" is True') # testing
    ...
    if opt.progressive_view:
        ...
        print('"opt.progressive_view" is True') # testing
    ...
    if opt.backbone == 'vanilla':
        ...
        print('vanilla backbone has been created') # testing
    elif opt.backbone == 'grid':
        # from nerf.network_grid import NeRFNetwork
        print('grid backbone has been created') # testing
    elif opt.backbone == 'grid_tcnn':
        # from nerf.network_grid_tcnn import NeRFNetwork
        print('grid_tcnn backbone has been created') # testing
    ...
    # device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # 나중에 해제
    print('device 설정 완료') # testing
    # model = NeRFNetwork(opt).to(device) # 나중에 해제
    print('Model 객체 생성') # testing

    if opt.dmtet and opt.init_ckpt != '':
        # load pretrained weights to init dmtet
        # state_dict = torch.load(opt.init_ckpt, map_location=device) # 나중에 해제
        # model.load_state_dict(state_dict['model'], strict=False) # 나중에 해제
        print('init checkpoint 로딩 완료') # testing
        if opt.cuda_ray:
            # model.mean_density = state_dict['mean_density'] # 나중에 해제
            print('cuda_ray 설정 완료') # testing
        # model.init_tet() # 나중에 해제
        print('model.init_tet() 실행 완료') # testing

    # print(model) # 나중에 해제
    print('Model 설정 완료') # testing

    if opt.test:
        print('"opt.test" is True') # testing
        print('test 모드') # testing
        # guidance = None # no need to load guidance model at test # 나중에 해제
        print('guidance 비활성화') # testing

        # trainer = Trainer(' '.join(sys.argv), 'df', opt, model, guidance, device=device, workspace=opt.workspace, fp16=opt.fp16, use_checkpoint=opt.ckpt) # 나중에 해제
        print('trainer 객체 생성') # testing
        ...
        if opt.gui:
            ...
            # test_loader = NeRFDataset(opt, device=device, type='test', H=opt.H, W=opt.W, size=100).dataloader() # 나중에 해제
            # trainer.test(test_loader) # 나중에 해제
            print('trainer.test() 실행(image to 3d)') # testing

            if opt.save_mesh:
                # trainer.save_mesh() 
                print('save_mesh() 실행 완료') # testing

    else:
        
        # train_loader = NeRFDataset(opt, device=device, type='train', H=opt.h, W=opt.w, size=100).dataloader() # 나중에 해제
        print('train 모드') # testing
        ...
        if opt.optim == 'adan':
            ...
            print('Adan 옵티마이저 객체 생성')  # testing
        else: # adam
            # optimizer = lambda model: torch.optim.Adam(model.get_params(opt.lr), betas=(0.9, 0.99), eps=1e-15)
            print('Adam 옵티마이저 객체 생성') # testing

        if opt.backbone == 'vanilla':
            # scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.1 ** min(iter / opt.iters, 1))
            print('opt.backbone is vanilla') # testing
            print('scheduler 객체 생성(LR 변동됨)') # testing
        else:
            # scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 1) # fixed
            print('opt.backbone is not vanilla') # testing
            print('fixed scheduler 객체 생성(LR 고정됨)') # testing
            # scheduler = lambda optimizer: optim.lr_scheduler.LambdaLR(optimizer, lambda iter: 0.1 ** min(iter / opt.iters, 1))

        if opt.guidance == 'stable-diffusion':
            # from guidance.sd_utils import StableDiffusion
            # guidance = StableDiffusion(device, opt.fp16, opt.vram_O, opt.sd_version, opt.hf_key, opt.t_range)
            print('stable diffusion guidance 객체 생성') # testing
        elif opt.guidance == 'zero123':
            # from guidance.zero123_utils import Zero123
            # guidance = Zero123(device, opt.fp16, opt.vram_O, opt.t_range)
            print('zero123 guidance 객체 생성') # testing
        elif opt.guidance == 'clip':
            # from guidance.clip_utils import CLIP
            # guidance = CLIP(device)
            print('clip guidance 객체 생성') # testing
        else:
        ...
        print('trainer 객체 생성 완료') # testing
        ...
        if opt.gui:
            ...
            print('Excuting trainer.train() ...') # testing
            # trainer.train(train_loader, valid_loader, max_epoch)

            # also test at the end
            # test_loader = NeRFDataset(opt, device=device, type='test', H=opt.H, W=opt.W, size=100).dataloader()
            print('Excuting trainer.test() ...') # testing
            # trainer.test(test_loader)

            if opt.save_mesh:
                print('Excuting trainer.save_mesh() ...') # testing
                # trainer.save_mesh()
